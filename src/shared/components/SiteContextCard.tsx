...
export const SiteContextCard: React.FC<SiteContextCardProps> = ({
    ...
    return (
        <div className={clsx('bg-surface-card border border-border-subtle rounded-xl overflow-hidden shadow-xs', className)}>
            ...
            <div className="p-4 space-y-3.5">
                ...
                <div className="pt-2 border-t border-border-subtle">
                    <button
                        ...
                        className="flex items-center justify-between w-full text-xs font-semibold text-text-secondary hover:text-text-primary transition-colors py-1"
                    >
                        <span className="flex items-center gap-1.5">
                            <ShieldCheck className="w-4 h-4 text-emerald-600" />
                            Source Provenance & Boundary Layer
                        </span>
                        {showMetadata ? <ChevronUp className="w-4 h-4 text-text-muted" /> : <ChevronDown className="w-4 h-4 text-text-muted" />}
                    </button>

                    {showMetadata && (
                        <div className="mt-2.5 p-3 rounded-lg bg-surface-well border border-border-subtle/80 space-y-2 text-xs">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <div>
                                    <span className="text-text-muted block text-[11px]">Layer Source:</span>
                                    <span className="font-medium text-text-primary">{site.sourceLayer.sourceName}</span>
                                </div>
                                <div>
                                    <span className="text-text-muted block text-[11px]">Source Date / Version:</span>
                            ...
                            <div>
                                <span className="text-text-muted block text-[11px]">Status & Reference:</span>
                                ...
                                <span className="ml-2 font-mono text-text-muted text-[11px]">{site.sourceLayer.crs}</span>
                            </div>

                            <div className="pt-1.5 border-t border-border-subtle/60 flex items-start gap-1.5 text-text-muted text-[11px]">
