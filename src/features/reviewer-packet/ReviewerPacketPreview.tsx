...
export const ReviewerPacketPreview: React.FC = () => {
  ...
  const packetData = useMemo(() => {
    ...
    // Use canonical contract builder — no recalculations performed
    return buildCanonicalReviewerPacketData(storeRecord);
  }, [caseId]);
  ...
  const classificationMeta =
    SPATIAL_CLASSIFICATIONS[packetData.spatialVerdict.classification] || {
      id: packetData.spatialVerdict.classification,
      badgeLabel: packetData.spatialVerdict.classification,
      badgeVariant: 'slate',
      summaryDescription: 'Classification pending.',
    };

  const statusMeta = CASE_STATUSES[packetData.currentStatus] || {
    id: packetData.currentStatus,
    label: packetData.currentStatus,
    badgeVariant: 'slate',
  ...
  const timelineEvents: TimelineEventItem[] = useMemo(() => {
    ...
    // If events exist in timeline, map them directly preserving immutable history
    if (packetData.eventsTimeline && packetData.eventsTimeline.length > 0) {
      return packetData.eventsTimeline.map((evt, idx) => {
        let timelineType: TimelineEventItem['eventType'] = 'STATUS_UPDATED';
        let role: TimelineEventItem['actorRole'] = 'SYSTEM';

        if (evt.eventType === 'OBSERVATION_CREATED') {
          timelineType = 'OBSERVATION_CREATED';
          role = 'REPORTER';
        } else if (evt.eventType === 'LOCATION_CAPTURED') {
          timelineType = 'LOCATION_CAPTURED';
          role = 'SYSTEM';
        } else if (evt.eventType === 'SPATIAL_CALCULATED') {
          timelineType = 'SPATIAL_EVALUATED';
          role = 'SYSTEM';
        } else if (evt.eventType === 'EVIDENCE_ATTACHED') {
          timelineType = 'EVIDENCE_ADDED';
          role = 'REPORTER';
        } else if (evt.eventType === 'REVIEW_ACTION_RECORDED') {
          role = 'REVIEWER';
          if (evt.actionTaken === 'ADDITIONAL_INFORMATION_NEEDED') {
            timelineType = 'INFO_REQUESTED';
          } else if (evt.actionTaken?.startsWith('CLOSED')) {
            timelineType = 'CASE_CLOSED';
          } else {
            timelineType = 'STATUS_UPDATED';
          }
        }

        const statusLabel = evt.resultingStatus
          ? (CASE_STATUSES[evt.resultingStatus]?.label || evt.resultingStatus)
          : 'REVIEW';

        return {
          id: evt.eventId || `evt-${idx}`,
          ...
          title: evt.summary || `Audit Event: ${evt.eventType}`,
          description:
            evt.reviewerNotes ||
            evt.summary ||
            'Audit trail event logged into immutable Change Ledger.',
          metadataBadge: statusLabel,
        };
      });
    }

    // Baseline fallback timeline
    return [
      {
        id: `${packetData.caseId}-pkt-1`,
        eventType: 'OBSERVATION_CREATED',
        actorRole: 'REPORTER',
        timestamp: new Date(packetData.generatedTimestamp).toLocaleString([], {
          dateStyle: 'short',
          timeStyle: 'short',
        }),
        title: 'Field Observation Created',
        description: `Observation logged under category "${packetData.categoryLabel || packetData.category}".`,
        metadataBadge: 'Field Capture',
      },
      {
        id: `${packetData.caseId}-pkt-2`,
        ...
        description: `Hardware GPS position (${packetData.coordinates.latitude.toFixed(5)}°N, ${packetData.coordinates.longitude.toFixed(5)}°E) with ±${packetData.coordinates.gpsAccuracyMeters.toFixed(1)}m uncertainty radius.`,
        metadataBadge: `±${packetData.coordinates.gpsAccuracyMeters.toFixed(1)}m error`,
      },
      {
        id: `${packetData.caseId}-pkt-3`,
        ...
        description: `Protected geometry layer (${packetData.provenance.sourceAgency}, ${packetData.provenance.bhuvanVersionStatement}) correlated.`,
        metadataBadge: packetData.provenance.crs || 'EPSG:4326',
      },
      {
        id: `${packetData.caseId}-pkt-4`,
        ...
        title: 'Spatial Engine Finding Recorded',
        description: `Classification: ${packetData.spatialVerdict.classification}. Distance: ${
          packetData.spatialVerdict.distanceToBoundaryMeters !== null
            ? `${packetData.spatialVerdict.distanceToBoundaryMeters.toFixed(1)} m`
            : 'N/A'
        }. Overlap flag: ${packetData.spatialVerdict.isUncertaintyOverlap ? 'Detected' : 'Disjoint'}.`,
        metadataBadge: packetData.spatialVerdict.classification,
      },
    ];
  }, [packetData]);

  // Latest Reviewer Decision & Administrative Rationale
  const latestReviewEvent = packetData.eventsTimeline?.find(
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 px-4 py-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-800 print:hidden">
        ...
        <div className="flex items-center gap-2">
          <Link to={`/case/${packetData.caseId}`}>
      ...
      {/* Legal Safety Advisory Banner (print:hidden) */}
        ...
        <div className="border-b-2 border-slate-900 pb-5 mb-6 flex items-start justify-between gap-4">
          ...
          <div className="text-right font-mono shrink-0">
            ...
            <div className="text-[10px] text-slate-500 mt-1">
              Generated: {packetData.generatedTimestamp.split('T')[0]}
            </div>
          </div>
        </div>

        {/* Printable Mandatory Legal Notice */}
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs bg-slate-50/80 p-4 rounded-lg border border-slate-200">
              <div>
                ...
                <div className="text-slate-600 text-[11px]">
                  {packetData.vernacularName} ({packetData.district}, {packetData.state})
                </div>
              </div>
              ...
              <div>
                ...
                <div className="font-semibold text-slate-800">
                  {packetData.provenance.sourceAgency}
                </div>
                <div className="text-slate-600 text-[11px] font-mono">
                  ASI Monument Code: {packetData.monumentNumber}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="font-mono font-medium text-slate-800">
                  {packetData.provenance.bhuvanVersionStatement} ({packetData.provenance.retrievalDate})
                </div>
              </div>
              ...
              <div className="sm:col-span-2 pt-2 border-t border-slate-200/80">
                ...
                <p className="text-[11px] text-slate-600 italic bg-white p-2.5 rounded border border-slate-200">
                  &quot;{packetData.provenance.verbatimLimitationText}&quot;
                </p>
              </div>
            </div>
          </section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-slate-50/80 p-3.5 rounded-lg border border-slate-200">
                <div>
                  ...
                  <div className="font-bold text-slate-900">
                    {packetData.categoryLabel || packetData.category}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="font-mono text-slate-800">
                    {new Date(packetData.generatedTimestamp).toLocaleString()}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="font-mono text-slate-800">
                    {packetData.coordinates.latitude.toFixed(5)}°N, {packetData.coordinates.longitude.toFixed(5)}°E (±{packetData.coordinates.gpsAccuracyMeters.toFixed(1)}m)
                  </div>
                </div>
              </div>
              ...
              {packetData.evidenceList && packetData.evidenceList.length > 0 ? (
                <div className="bg-slate-50/80 p-3.5 rounded-lg border border-slate-200 flex flex-col sm:flex-row items-start gap-4">
                  ...
                  <div className="space-y-1.5 text-xs">
                    ...
                    <div className="font-mono text-slate-800">
                      File MIME: <strong>{packetData.evidenceList[0].fileMimeType || 'image/jpeg'}</strong>
                    </div>
                    <div className="font-mono text-slate-800">
                      Size: <strong>{Math.round(packetData.evidenceList[0].fileSizeBytes / 1024)} KB</strong>
                    </div>
                    <div className="font-mono text-slate-800">
                      SHA-256 Digest: <strong className="text-[11px] break-all text-slate-700">{packetData.evidenceList[0].sha256Checksum}</strong>
                    </div>
                    <div className="font-mono text-slate-800">
                      Captured Date: <strong>{new Date(packetData.evidenceList[0].uploadTimestamp).toLocaleString()}</strong>
                    </div>
              ...
              <div className="bg-slate-50/80 p-4 rounded-lg border border-slate-200 space-y-3">
                <div className="flex items-center justify-between gap-2 border-b border-slate-200 pb-2">
                  ...
                  <Badge variant={classificationMeta.badgeVariant as any}>
                    {packetData.spatialVerdict.classification}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 font-mono">
                  ...
                  <div className="bg-white p-2.5 rounded border border-slate-200">
                    ...
                    <div className="font-bold text-slate-900 text-sm">
                      ±{packetData.coordinates.gpsAccuracyMeters.toFixed(1)} m
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-2.5 rounded border border-slate-200">
                    ...
                    <div className="font-bold text-slate-900 text-sm">
                      {packetData.spatialVerdict.isUncertaintyOverlap ? 'DETECTED' : 'DISJOINT'}
                    </div>
                  </div>
                </div>

                {packetData.spatialVerdict.uncertaintyReason && (
                  <div className="p-2.5 rounded bg-amber-50/80 border border-amber-200 text-amber-900 text-xs">
                    <span className="font-semibold block text-[10px] uppercase font-mono">Uncertainty Reason:</span>
                    <p>{packetData.spatialVerdict.uncertaintyReason}</p>
                  </div>
                )}
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-700 leading-relaxed font-sans bg-white p-3 rounded border border-slate-200">
                    {packetData.spatialVerdict.explanation}
                  </p>
                </div>

                {packetData.spatialVerdict.statements && (
                  <div className="grid grid-cols-1 gap-2 pt-2 border-t border-slate-200">
                    <div className="p-2.5 bg-white rounded border border-slate-200 text-[11px]">
                      <span className="text-slate-500 font-mono text-[10px] uppercase block font-semibold">
                        GIS-Calculated Statement:
                      </span>
                      <span className="text-slate-800">{packetData.spatialVerdict.statements.gisCalculated}</span>
                    </div>
                    <div className="p-2.5 bg-white rounded border border-slate-200 text-[11px]">
                      <span className="text-slate-500 font-mono text-[10px] uppercase block font-semibold">
                        Authority / Source Notice:
                      </span>
                      <span className="text-slate-800">{packetData.spatialVerdict.statements.authorityNotice}</span>
                    </div>
                  </div>
                )}

                {(packetData.spatialVerdict.isUncertaintyOverlap || packetData.coordinates.gpsAccuracyMeters > 35) && (
          ...
          <div className="pt-6 border-t-2 border-slate-900 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-[10px] text-slate-500 font-mono">
            ...
            <div className="text-right">
              Document Ref: HP-PACKET-{packetData.caseId}
              <br />
              Status: {packetData.currentStatus}
            </div>
          </div>
        </div>
      </div>
